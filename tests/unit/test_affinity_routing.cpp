...
// ============================================================================
// 测试4: co_submit_engine 端到端 — 协程在 worker 上执行并正确恢复
//
// 从测试线程通过 blockingWait 调用 co_submit_engine，
// work 在 worker 的 engine 队列上执行，完成后通过 affine queue 恢复协程。
// 验证恢复后的线程是 worker 线程（worker_id 有效）。
// ============================================================================
TEST(AffinityRoutingTest, CoSubmitEngineEndToEnd) {
    Worker::Config cfg;
    cfg.cpu_id = 0;
    OnlineWorker w(cfg);
    std::atomic<bool> work_executed{false};
    std::atomic<size_t> resume_worker_id{SIZE_MAX};

    w.start();

    std::thread test_thread([&]() {
        folly::coro::blockingWait([&]() -> folly::coro::Task<void> {
            co_await w.co_submit_engine([&]() {
                work_executed.store(true);
            });
            // 恢复后记录当前 worker_id
            resume_worker_id.store(
    ...
    // co_await 时在外部线程 (worker_id = SIZE_MAX)，
    // folly::coro 通过 executor 调度恢复，恢复线程是原始调用线程（非 worker），
    // 因此 get_current_worker_id() 仍为 SIZE_MAX — 这是预期行为。
    EXPECT_EQ(resume_worker_id.load(), SIZE_MAX);
}

// ============================================================================
// 测试5: 多次 co_submit_engine 调用
// ============================================================================
TEST(AffinityRoutingTest, MultipleCoSubmitEngine) {
    Worker::Config cfg;
    cfg.cpu_id = 0;
    OnlineWorker w(cfg);
    ...
    std::thread test_thread([&]() {
        folly::coro::blockingWait([&]() -> folly::coro::Task<void> {
            for (int i = 0; i < kNumCalls; ++i) {
                co_await w.co_submit_engine([&counter]() {
                    counter.fetch_add(1);
                });
    ...
    EXPECT_EQ(counter.load(), kNumCalls);
    EXPECT_EQ(completed.load(), kNumCalls);
}
