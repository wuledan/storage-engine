...
void rw_test_callback(void* ctx, IOCompletion comp) {
    auto* tc = static_cast<RWTestCtx*>(ctx);
    if (tc->result) *tc->result = comp;
    if (tc->done) tc->done->store(true);
}
...
void many_io_callback(void* ctx, IOCompletion comp) {
    auto* tc = static_cast<ManyIOCtx*>(ctx);
    if (comp.result > 0) {
        tc->total_bytes->fetch_add(static_cast<size_t>(comp.result));
    }
    tc->completed->fetch_add(1);
}
...
TEST(IOUringTest, ReadWriteFile) {
    ...
    RWTestCtx wctx{&write_done, &write_comp};
    write_req.callback_fn = rw_test_callback;
    write_req.callback_ctx = &wctx;
    ...
    RWTestCtx rctx{&read_done, &read_comp};
    read_req.callback_fn = rw_test_callback;
    read_req.callback_ctx = &rctx;
...
TEST(IOUringTest, ManyIOOps) {
    ...
    ManyIOCtx ctx{&completed, &total_bytes};
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        req.buf = buf;
        req.len = 64;
        req.callback_fn = many_io_callback;
