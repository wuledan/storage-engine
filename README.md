...
整个技术栈的底层执行引擎。C++20 无阻塞协程运行时，对齐 pthread 语义。
...
|------|------|
| `yield()` | 让出 CPU, 回优先级队列 |
| `Baton` | 协程间信号, 支持 wait_for(timeout) |
| `Mutex` / `Semaphore` | 互斥/计数, 亲和性路由 |
| `co_sleep_for` | 定时挂起, Online/Offline 自动路由 |
| `coro_thread` | pthread 等价, join/detach/yield |

**pthread 迁移**：`pthread_create` → `coro_thread`, `pthread_mutex_lock` → `co_await mtx.co_lock()`, `sleep` → `co_await co_sleep_for()`。详见 [协程使用指南](docs/2026-06-10/26-coroutine-usage.md)。

**禁止**：`folly::coro::blockingWait`, `std::mutex`, `std::condition_variable` — 破坏协作调度。
