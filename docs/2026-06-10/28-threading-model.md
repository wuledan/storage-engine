...
| 每任务 CPU 时间 | 1核 IOPS | 2核 | 4核 | 8核 | 扩展比 |
|----------------|----------|-----|-----|-----|--------|
| 10μs | 95K | 182K | 358K | 702K | ~1.85x |
| 50μs | 19.7K | 39.5K | 79K | 157K | ~2.0x |
| 100μs | 9.9K | 19.9K | 39.8K | 79.3K | ~2.0x |

- 纯计算任务近完美线性扩展
- 偷取效率：1000 tasks×200μs 全推 worker-0，worker-1 偷走 500
- NUMA-aware 偷取限制在同节点，跨节点不偷（避免远程内存访问）
