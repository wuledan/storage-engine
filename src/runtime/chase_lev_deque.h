...
//
// Key characteristics:
//   - Template class ChaseLevDeque<T>
//   - Dynamic circular array with doubling
//   - push(T) — owner bottom push, non-atomic
//   - pop(T&) — owner bottom pop, atomic CAS on contention with thief
//   - steal(T&) — thief top steal, atomic CAS
//   - Default capacity 256 items
//
// Synchronization is via atomic top_/bottom_ indices; the data array itself
// is a plain vector<T> (not atomic<T>). This avoids the need for lock-free
// atomic<T> on the item type.
template<typename T>
class ChaseLevDeque {
public:
    ...
    // On contention with a concurrent steal(), the thief may win and pop() returns false.
    bool pop(T& out) {
        ...
        if (t <= b) {
            ...
            if (t == b) {
                ...
                // Attempt to advance top so the item is claimed.
                if (!top_.compare_exchange_strong(t, t + 1,
                        std::memory_order_seq_cst, std::memory_order_relaxed)) {
                    // Thief won — restore bottom so top == bottom (empty).
                    bottom_.store(b + 1, std::memory_order_relaxed);
                    return false;
                }
                // CAS succeeded — we claimed the last item.
                // Restore bottom so top == bottom (empty).
                bottom_.store(b + 1, std::memory_order_relaxed);
            }
            // t < b: bottom_ stays at b (already decremented for the new bottom).
            return true;
        }

        // Empty — nothing to pop, restore bottom.
        bottom_.store(b + 1, std::memory_order_relaxed);
        return false;
    }

    // Any thread: steal item from top (FIFO).
    // Returns true if an item was successfully stolen.
    bool steal(T& out) {
        int64_t t = top_.load(std::memory_order_acquire);
        std::atomic_thread_fence(std::memory_order_seq_cst);
